...
@Service
public class SetupOperationsImpl implements SetupOperations {
    ...
    public void annotateType(JavaType javaType) {
        ...
        if (existing != null && MemberFindingUtils.getAnnotationOfType(existing.getAnnotations(), new JavaType(RooSetup.class.getName())) == null) {
            ...
            JavaType rooRooSetup = new JavaType(RooSetup.class.getName());

            // Create Annotation metadata
            AnnotationMetadataBuilder annotationBuilder = new AnnotationMetadataBuilder(rooRooSetup);
    ...
    public void setup() {
        ...
        projectOperations.addRepository("", new Repository("Setup Roo add-on repository", "Setup Roo add-on repository", "https://migration-setup.googlecode.com/svn/repo"));
        
        List<Dependency> dependencies = new ArrayList<Dependency>();
        
        // Install the dependency on the add-on jar (
        dependencies.add(new Dependency("cz.cvut.valespe.migration.setup", "cz.cvut.valespe.migration.setup", "0.1.0.BUILD-SNAPSHOT", DependencyType.JAR, DependencyScope.PROVIDED));
