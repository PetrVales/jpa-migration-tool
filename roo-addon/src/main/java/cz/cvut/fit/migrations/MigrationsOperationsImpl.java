...
@Service
public class MigrationsOperationsImpl implements MigrationsOperations {
    ...
    public void annotateType(JavaType javaType) {
        ...
        if (existing != null && MemberFindingUtils.getAnnotationOfType(existing.getAnnotations(), new JavaType(RooMigrations.class.getName())) == null) {
            ...
            JavaType rooRooMigrations = new JavaType(RooMigrations.class.getName());

            // Create Annotation metadata
            AnnotationMetadataBuilder annotationBuilder = new AnnotationMetadataBuilder(rooRooMigrations);
    ...
    public void setup() {
        ...
        projectOperations.addRepository("", new Repository("Migrations Roo add-on repository", "Migrations Roo add-on repository", "https://jpa-migration-addon.googlecode.com/svn/repo"));
        
        List<Dependency> dependencies = new ArrayList<Dependency>();
        
        // Install the dependency on the add-on jar (
        dependencies.add(new Dependency("cz.cvut.fit.migrations", "cz.cvut.fit.migrations", "0.1.0.BUILD-SNAPSHOT", DependencyType.JAR, DependencyScope.PROVIDED));
